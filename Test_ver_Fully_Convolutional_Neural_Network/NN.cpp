...
void NeuralNetwork::initialize(const VectorT<unsigned>& num_layer_acts_, const int& _num_hidden_layers)
{
	...
	{
		weights_[l].initialize(layer_neuron_act_[l + 1].num_dimension_ - 1, layer_neuron_act_[l].num_dimension_);//y = W [x b]^T
...
D NeuralNetwork::getSigmoidGradFromY(const D& y)   //y = getSigmoid(x).
{
	return (1.0 - y) * y;
}
...
D NeuralNetwork::getLRELUGradFromY(const D& x)
{
	if (x > 0.0) return 1.0;
	else return 0.01;
}

void NeuralNetwork::applySigmoidToVector(VectorT<D>& vector)
{
	for (int d = 0; d < vector.num_dimension_ - 1; d++)
		vector[d] = getSigmoid(vector[d]);
}

void NeuralNetwork::applyRELUToVector(VectorT<D>& vector)
{
	for (int d = 0; d < vector.num_dimension_ - 1; d++)
		vector[d] = getRELU(vector[d]);
}

void NeuralNetwork::applyLRELUToVector(VectorT<D>& vector)
{
	for (int d = 0; d < vector.num_dimension_ - 1; d++)
...
void NeuralNetwork::propBackward(const VectorT<D>& target)
{
	...
	for (int l = weights_.num_elements_ - 1; l >= 0; l--)
	{
		...
		for (int d = 0; d < layer_neuron_act_[l].num_dimension_ - 1; d++)
		{
...
void NeuralNetwork::updateWeight(MatrixTest<D>& weight_matrix, VectorT<D>& next_layer_grad, VectorT<D>& prev_layer_act)
{
