...
public:
	...
	Arr1D(const int& num_elements_input)
		: num_elements_(0), values_(nullptr)
	{
	...
	Arr1D(const Arr1D<TT>& array_input)
		: num_elements_(0), values_(nullptr)
	{
	...
	void copyFrom(const Arr1D<TT>& from)
	{
	...
	void copyToPartialArray(Arr1D<TT>& dest, const int start, const int end) {
	...
	void append(const Arr1D<TT>& source)
	{
	...
	friend std::ostream& operator<< (std::ostream& stream, const Arr1D<TT>& arr)
	{
