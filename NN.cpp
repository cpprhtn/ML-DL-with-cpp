...
NN::NN(const int& _num_input, const int& _num_output, const int& _num_hidden_layers)
{
	initialize(_num_input, _num_output, _num_hidden_layers);
}

void NN::initialize(const int& _num_input, const int& _num_output, const int& _num_hidden_layers)
{
...
void NN::initialize(const VectorT<unsigned>& num_layer_acts_, const int& _num_hidden_layers)
{
...
D NN::getSigmoid(const D& x)
{
	return 1.0 / (1.0 + exp(-x));
}

D NN::getSigmoidGrad(const D& y)   //y = getSigmoid(x)
{
	return (1.0 - y) * y;
}

D NN::getRELU(const D& x)
{
	return MAX2(0.0, x);
}

D NN::getRELUGrad(const D& x) //RELU Grad from X == RELU Grad from Y
{
	if (x > 0.0) return 1.0;
	else return 0.0;
}

D NN::getLRELU(const D& x)
{
	return x > 0.0 ? x : 0.01*x;
}

D NN::getLRELUGrad(const D& x) //RELU Grad from X == RELU Grad from Y
{
	if (x > 0.0) return 1.0;
	else return 0.01;
}

void NN::SigmoidToVector(VectorT<D>& vector)
{
	for (int d = 0; d < vector.num_dimension - 1; d++) 
		vector[d] = getSigmoid(vector[d]);
}

void NN::RELUToVector(VectorT<D>& vector)
{
	for (int d = 0; d < vector.num_dimension - 1; d++)
		vector[d] = getRELU(vector[d]);
}

void NN::LRELUToVector(VectorT<D>& vector)
{
	for (int d = 0; d < vector.num_dimension - 1; d++)
...
void NN::propBackward(const VectorT<D>& target)
{
...
void NN::updateWeight(MatrixT<D>& weight_matrix, VectorT<D>& next_layer_grad, VectorT<D>& prev_layer_act)
{
...
void NN::setInputVector(const VectorT<D>& input)
{
...
void NN::copyOutputVector(VectorT<D>& copy, bool copy_bias)
{
